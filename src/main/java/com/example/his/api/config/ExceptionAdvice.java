...
public class ExceptionAdvice {
     ...
     * 捕获异常，并且返回500状态码
     */
    @ResponseBody // 结果变为JSON格式
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR) // 响应状态码为 401
    @ExceptionHandler(Exception.class) // 捕获 Exception 异常
    public String exceptionHandler(Exception e) {
        ...
        if (e instanceof HttpMessageNotReadableException) {
            ...
            json.set("error", "请求未提交数据或者提交数据有误");
        }
        // 2. 前端上送文件时，上传文件不对
     ...
     */
    @ResponseBody // 结果变为JSON格式
    @ResponseStatus(HttpStatus.UNAUTHORIZED) // 响应状态码为 401
    @ExceptionHandler(NotLoginException.class) // 捕获 NotLoginException 异常
